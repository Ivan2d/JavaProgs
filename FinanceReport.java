...
public class FinanceReport
{
    ...
    public String toString()
    {
        ...
        String last = "ФИО = %s, day = %d, Mounth = %d, year = %d, sumPayment = %f";
        for(int i = 0; i < Reportlength(); i++)
        {
            if(i == Reportlength()-1)
            {
                l.append(String.format(last, report[i].getFio(), report[i].getDay(), report[i].getMonth(), report[i].getYear(), report[i].getPaysum()));
            }
            else
            {
